...
create_comprehensive_test_db <- function(n_rows = 3) {
  ...
  for (i in 1:n_rows) {
    ...
    # Generate parameters
    base_params <- sprintf("[%.2f, %.2f, %.2f]", 
                          runif(1), runif(1), runif(1))
    random_params <- sprintf("[%.2f, %.2f, %.2f]", 
                            runif(1), runif(1), runif(1))
    ...
    dbExecute(conn, "INSERT INTO parameters VALUES (?, 'truthy', ?)", 
             params = list(truthy_id, base_params))  # Same as base
...
validate_processed_data_structure <- function(df) {
  ...
  expected_measures <- c("agent_time_enableds", "collisionTimes", 
